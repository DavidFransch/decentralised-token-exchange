...
contract('Exchange', ([deployer, feeAccount, user1, user2])=>{
    ...
    describe('withdrawing Ether', async()=>{
        ...
        describe('success', async()=>{
            ...
            //Check that Ether has been taken out of token mapping
            ...
            it('emits a Withdraw event', async()=>{
                ...
                event.user.should.equal(user1, 'user address is correct')
                event.amount.toString().should.equal(amount.toString(), 'amount is correct')
                event.balance.toString().should.equal('0', 'balance is 0')    
            })
        })
      ...
      await exchange.makeOrder(token.address, tokens(1), ETHER_ADDRESS, ether(1), { from: user1 })
        ...
        describe('filling orders', async()=>{
            ...
            describe('failure', async()=>{
                ...
                it('rejects already filled ordeers', async()=>{
                    await exchange.fillOrder('1', {from:user2}).should.be.fulfilled
                    //Try to cancel order from another user
                    await exchange.fillOrder('1', { from:user2}).should.be.rejectedWith(EVM_REVERT)
                })
                it('rejects cancelled ordeers', async()=>{
                    //Cancel order
